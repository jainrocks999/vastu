...
use Botble\Ecommerce\Services\CreatePaymentForOrderServiceApi;
...
class ApiOrderController extends Controller
{
    ...
    public function fetchOrderDetails(Request $request){
        try{
            ...
            $response = ['status'=>200,'data'=>$orders,'msg'=>"Fetch Order details."];
            return response($response, 200);
        } catch (\Exception $e) {
            // Handle exceptions
            return response()->json(['status' => 500, 'msg' => 'Something went wrong.'], 500);
    ...
    //Order create
    public function createOrder(Request $request, CreatePaymentForOrderServiceApi $CreatePaymentForOrderServiceApi){
        try{
            $validator = Validator::make($request->all(), [
                'user_id' => 'required|integer|max:255', 
            ]);
            if ($validator->fails()) {
                ...
                if (Arr::get($data, 'error')) {
                    $response =['status'=>400,'msg' => implode('; ', Arr::get($data, 'message', []))];
                    return response($response, 200);
                }
                ...
                $request->merge([
                    ...
                    'user_id' => $customerId,
                    'shipping_method' => $request->shipping_method ? $request->shipping_method : ShippingMethodEnum::DEFAULT,
                    'shipping_option' => $request->shipping_option ? $request->shipping_option : '',
                    'shipping_amount' => $request->shipping_amount ? $request->shipping_amount: 0 ,
                    'tax_amount' => $request->tax_amount ? $request->tax_amount: 0,
                    'sub_total' => $request->sub_amount ? $request->sub_amount: 0,
                    'coupon_code' => $request->coupon_code,
                    'discount_amount' => $request->discount_amount ? $request->discount_amount: 0,
                    'promotion_amount' => $request->promotion_amount ? $request->promotion_amount: 0,
                    'discount_description' => $request->discount_description ? $request->discount_description : 0 ,
                    'description' => $request->note ? $request->note : 0 ,
                    'is_confirmed' => 1,
                    'is_finished' => 1,
                ...
                if ($order) {
                    OrderHistory::query()->create([
                        'action' => 'Order was created from mobile Application',
                    ...
                    $CreatePaymentForOrderServiceApi->execute(
                        $order,
                    ...
                    $products = DB::table('ec_cart_temp')->where('user_id',$request->user_id)->get()->toArray();
                  //  $tempCartdesroy = DB::table('ec_cart_temp')->where('user_id',$request->user_id)->where('rowid',$productItem->rowid)->delete();
                    foreach ($products as $productItem) {
                        $product_option = json_decode($productItem->option);
                        $orderProduct = [
                            'order_id' => $order->id,
                            'product_id' => $productItem->product_id,
                            'product_name' => $productItem->name,
                            'product_image' => $product_option->image,
                            'qty' => $productItem->qty,
                            'weight' => 0,
                            'price' => $productItem->price,
                            'tax_amount' => $productItem->taxrate,
                            'product_options' => '',
                            'options' => json_encode($product_option),
                            'product_type' => 'physical',
                        ];
        
                        $products =  OrderProduct::query()->create($orderProduct);
                        $tempCartdesroy = DB::table('ec_cart_temp')->where('rowid',$productItem->rowid)->delete();
        ...
        try{
            ...
            if ($validator->fails()) {
                ...
                $orders = Order::query()
                        ->where('user_id', $request->user_id)
                        ->where('id', $request->order_id)
                        ->where('code', $request->order_number)
                        ->first();
                if(!empty($orders) && !empty($orders)){
                    $orders->status = 'cancel';
                    $order->description = $request->reasion;
                    $order->save();
                }
            }
            $response = ['status'=>200,'data'=>$orders,'msg'=>"Order cencel successfully.."];
            return response($response, 200);
        } catch (\Exception $e) {
            // Handle exceptions
            return response()->json(['status' => 500, 'msg' => 'Something went wrong.'], 500);
    ...
    // public function postCreateShipment(Order $order, Request $request)
    // {
    //     $shipment = [
    //         'order_id' => $order['id'],
    ...
    //         ]);
    //   //  }
    //     $response = ['status'=>200,'data'=>$result,'msg'=>"Fetch Order details."];
    //     return response($response, 200);
    // }
    ...
    public function getAvailableShippingMethods(Request $request, HandleShippingFeeService $shippingFeeService)
    {
        ...
        try{
            ...
            if ($validator->fails()) {
                ...
                $response = ['status'=>200,'data'=>$result,'msg'=>"Fetch Shipping Method list."];
                return response($response, 200);
            }
        } catch (\Exception $e) {
            // Handle exceptions
    ...
    //     CreatePaymentForOrderServiceApi $CreatePaymentForOrderServiceApi
    // ) {
    //     DB::transaction(function () use ($order, $CreatePaymentForOrderServiceApi, $request) {
    //         /** @var User $user */
    //         $user = Auth::user();
    ...
    protected function getDataBeforeCreateOrder(Request $request): array
    {
        ...
        if ($productIds = $inputProducts->pluck('product_id')->all()) {
            $products = Product::query()
                ->whereIn('id', $productIds)
                ->with($with)
                ->get();
