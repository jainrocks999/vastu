        ...
        $this
                ...
                $this
                    ->add(
                        ...
                        SelectFieldOption::make()
                            ...
                            ->choices($brands)
                            ->selected(old('categories', $brands))
                            // ->searchable()
            ...
            ->add(
                ...
                MediaImageFieldOption::make()
                    ...
                    ->helperText(trans('plugins/ecommerce::products.form.featured_image_helper'))
            )
          ...
          ->when(EcommerceHelper::isTaxEnabled(), function () {
                    ...
                    if ($taxes) {
                        ...
                        if ($product && $product->getKey()) {
                            $selectedTax = $product->taxes()->pluck('tax_id')->first(); // Get the first tax ID if any
                        } elseif ($defaultTaxRate = get_ecommerce_setting('default_tax_rate')) {
                            $selectedTax = $defaultTaxRate; // Use default tax rate if set
                        }
                
                        $this->add('taxes', SelectField::class, [
                            'label' => trans('plugins/ecommerce::products.form.taxes'),
                            'choices' => $taxes,
                            'value' => old('taxes', $selectedTax),
                        ]);
                    }
                })
