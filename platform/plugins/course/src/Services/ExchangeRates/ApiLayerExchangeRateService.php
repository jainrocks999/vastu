...
use Botble\Course\Facades\Currency;
use Botble\Course\Models\Currency as CurrencyModel;
use Exception;
...
class ApiLayerExchangeRateService implements ExchangeRateInterface
{
    ...
    {
        if (! get_course_setting('api_layer_api_key')) {
            throw new Exception(trans('plugins/course::currency.no_api_key'));
        }
    ...
    protected function request(array $params): array
    {
        $response = Http::baseUrl('https://api.apilayer.com')
            ->withoutVerifying()
            ->withHeaders(['apikey' => get_course_setting('api_layer_api_key')])
            ->acceptJson()
