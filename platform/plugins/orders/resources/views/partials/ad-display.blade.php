@foreach($data as $item)
    @if ($item->orders_type === 'google_ordersense' && $item->google_ordersense_slot_id)
        <div {!! Html::attributes($attributes) !!}>
            @include('plugins/orders::partials.google-ordersense.unit-orders-slot', ['slotId' => $item->google_ordersense_slot_id])
        </div>
        @continue
    @endif
