...
namespace Botble\Orders\Models;
...
class Orders extends BaseModel
{
    ...
    protected function imageUrl(): Attribute
    {
        return Attribute::get(
            function (): string {
                if (config('plugins.orders.general.use_real_image_url')) {
    ...
    protected function clickUrl(): Attribute
    {
        return Attribute::get(
            fn ($_, array $attributes = []): string =>
                route('public.orders-click.alternative', [
                    'randomHash' => $this->random_hash,
                    'ordersKey' => $attributes['key'],
                ])
        );
    }
    ...
    {
        return route('public.orders-click.image', [
            'randomHash' => $this->random_hash,
            'ordersKey' => $this->key,
            'size' => $size,
