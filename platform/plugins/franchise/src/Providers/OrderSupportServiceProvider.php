...
use Botble\Franchise\Models\CategoryCommission;
...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function renderProductsInCheckoutPage(array|string|EloquentCollection $products): string|array|Collection
    {
        if ($products instanceof Collection) {
            ...
            return view(
                'plugins/franchise::orders.checkout.products',
    ...
    public function processPostCheckoutOrder(
        ...
        $mpSessionData = Arr::get($sessionCheckoutData, 'franchise', []);
        ...
        if ($couponCode) {
            ...
            $sessionCheckoutData = OrderHelper::setOrderSessionData($token, ['franchise' => $mpSessionData]);
        }
    ...
    public function processApplyCouponCode(array $result, Request $request): array
    {
        ...
        $sessionCheckoutData = OrderHelper::getOrderSessionData($token);
        $sessionFranchiseData = Arr::get($sessionCheckoutData, 'franchise', []);
        $results = collect();
        ...
        foreach ($groupedProducts as $storeId => $groupedProduct) {
            ...
            $sessionData = Arr::get($sessionFranchiseData, $storeId, []);
            $prefix = "franchise.$storeId.";
        ...
        foreach ($results as $storeId => $result) {
            ...
            Arr::set($sessionFranchiseData, $storeId, $sessionData);
        }
        ...
        $couponDiscountAmount = collect($sessionFranchiseData)->sum('coupon_discount_amount');

        OrderHelper::setOrderSessionData($token, [
            'franchise' => $sessionFranchiseData,
    ...
    public function handleCheckoutOrderByStore(
        ...
        $promotionDiscountAmount = $promotionService
            ->execute($token, compact('cartItems', 'rawTotal', 'countCart'), "franchise.$storeId.");
    ...
    public function processGetCheckoutSuccess(string $token)
    {
        ...
        return view('plugins/franchise::orders.thank-you', compact('orders'));
    }
    ...
    public function sendMailAfterProcessOrder(Collection $orders): Collection
    {
        ...
        FranchiseHelper::sendMailToVendorAfterProcessingOrder($orders);
    ...
    public function setEmailVariables(Collection $orders): \Botble\Base\Supports\EmailHandler
    {
        ...
        return EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME)
            ->setVariableValues([
                ...
                'product_list' => view('plugins/franchise::emails.partials.order-detail', compact('orders'))
                    ->render(),
    ...
    public function processShippingDiscountOrderData(
        ...
        $mpSessionCheckoutData = (array) Arr::get($sessionCheckoutData, 'franchise', []);
        ...
        if ($couponCode) {
            ...
            $sessionCheckoutData['franchise'] = $mpSessionCheckoutData;

            OrderHelper::setOrderSessionData($token, $sessionCheckoutData);
        }

        $mpSessionCheckoutData = Arr::get($sessionCheckoutData, 'franchise');
        $discounts = collect();

        $franchiseData = collect();
        ...
        foreach ($groupedProducts as $storeId => $productsByStore) {
            ...
            $prefixPromotion = "franchise.$storeId.";
            ...
            $sessionCheckoutData['franchise'] = [$storeId => $vendorSessionData];
        ...
        $shippingAmount = $franchiseData->pluck('shipping_amount')->sum();
        $promotionDiscountAmount = $franchiseData->pluck('promotion_discount_amount')->sum();
        $couponDiscountAmount = $franchiseData->pluck('coupon_discount_amount')->sum();
        ...
        foreach ($mpSessionCheckoutData as $storeId => $mpSessionData) {
            Arr::set(
                ...
                $storeId,
                array_merge($mpSessionData, Arr::get($franchiseData, $storeId, []))
            );
        }

        $sessionCheckoutData = OrderHelper::mergeOrderSessionData($token, ['franchise' => $mpSessionCheckoutData]);
        $sessionCheckoutData['is_available_shipping'] = $franchiseData->where('is_available_shipping')->count();
    ...
    public function processPostSaveInformation(array $sessionCheckoutData, Request $request, string $token): array
    {
        ...
        Arr::set($sessionCheckoutData, 'franchise', $mpSessionData);
    ...
    public function handleProcessOrder(
        ...
        $preOrders = collect();
        $mpSessionData = Arr::get($sessionData, 'franchise', []);
        ...
        $sessionData = array_merge($sessionData, ['franchise' => $mpSessionData]);
    ...
    public function processCheckoutMessagesRequest(array $messages): array
    {
        ...
        foreach ($stores as $storeId => $storeName) {
            $messages["shipping_method.$storeId.required"] = trans(
                'plugins/franchise::order.shipping_method_required',
                ['name' => $storeName]
            );
            $messages["shipping_method.$storeId.in"] = trans(
                'plugins/franchise::order.shipping_method_in',
                ['name' => $storeName]
            );
            $messages["shipping_option.$storeId.required"] = trans(
                'plugins/franchise::order.shipping_option_required',
                ['name' => $storeName]
            );
        }
    ...
    public function afterOrderStatusCompleted(Order $order)
    {
        ...
        if ($order->store->id && $order->store->customer->id) {
            ...
            if ($vendorInfo->id) {
                ...
                if (! FranchiseHelper::isCommissionCategoryFeeBasedEnabled()) {
                    $feePercentage = FranchiseHelper::getSetting('fee_per_order', 0);
    ...
    protected function calculatorCommissionFeeByProduct(Collection $orderProducts): float|int
    {
        ...
        foreach ($orderProducts as $orderProduct) {
            ...
            $commissionFeePercentage = FranchiseHelper::getSetting('fee_per_order', 0);
        ...
        if ($order && $order->store->id && $order->store->customer->id) {
            ...
            if ($vendorInfo->id) {
                ...
                $data = [
                    ...
                    'description' => trans('plugins/franchise::order.return.description', [
                        'order' => $order->code,
                    ]),
                ];
