...
use Botble\Franchise\Forms\PayoutInformationForm;
use Botble\Franchise\Forms\TaxInformationForm;
use Botble\Franchise\Forms\VendorStoreForm;
use Botble\Franchise\Http\Requests\Fronts\VendorStoreRequest;
use Botble\Franchise\Http\Requests\PayoutInformationSettingRequest;
use Botble\Franchise\Http\Requests\TaxInformationSettingRequest;
...
class SettingController extends BaseController
{
    public function index()
    {
        ...
        $taxInformationForm = TaxInformationForm::createFromModel($store->customer)
            ->setUrl(route('franchise.vendor.settings.post.tax-info'))
            ->renderForm();

        $payoutInformationForm = PayoutInformationForm::createFromModel($store->customer)
            ->setUrl(route('franchise.vendor.settings.post.payout'))
            ->renderForm();

        return FranchiseHelper::view(
    ...
    public function saveSettings(VendorStoreRequest $request)
    {
        ...
        $storeForm->saving(function (VendorStoreForm $form) use ($request) {
            ...
            if ($request->has('social_links')) {
                if ($socialLinks = $request->input('social_links', [])) {
                    $socials = array_keys(FranchiseHelper::getAllowedSocialLinks());
        ...
        return $this
            ->httpResponse()
            ->setNextUrl(route('franchise.vendor.settings'))
    ...
    public function updateTaxInformation(TaxInformationSettingRequest $request)
    {
        ...
        event(new UpdatedContentEvent(CONSULTATION_MODULE_SCREEN_NAME, $request, $store));
