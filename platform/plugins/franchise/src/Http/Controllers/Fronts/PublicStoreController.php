...
use Botble\Franchise\Forms\ContactStoreForm;
use Botble\Franchise\Http\Requests\Fronts\CheckStoreUrlRequest;
...
class PublicStoreController extends BaseController
{
    public function getStores(Request $request)
    {
        ...
        return Theme::scope('franchise.stores', compact('stores'), FranchiseHelper::viewPath('stores', false))->render();
    }
    ...
    public function getStore(
        ...
        if ($request->ajax()) {
            ...
            $view = Theme::getThemeNamespace('views.franchise.stores.items');

            if (! view()->exists($view)) {
                $view = FranchiseHelper::viewPath('stores.items', false);
            }
        ...
        if (function_exists('admin_bar')) {
            admin_bar()
                ->registerLink(
                    trans('plugins/franchise::store.edit_this_store'),
                    route('franchise.store.edit', $store->getKey()),
                    null,
                    'franchise.store.edit'
                );
        }
        ...
        return Theme::scope(
            ...
            FranchiseHelper::viewPath('store', false)
        )->render();
    }
