...
namespace Botble\Franchise\Http\Controllers\Customers;
...
use Botble\Fra\Forms\FranchiseForm;
...
class FranchiseCustomerController extends BaseController
{
    protected function breadcrumb(): Breadcrumb
    {
        return parent::breadcrumb()
            ->add(trans('plugins/franchise::franchise.name'), route('franchise.customers.index'));
    }

    public function index(FranchiseTable $dataTable)
    {
        $this->pageTitle(trans('plugins/franchise::customer.name'));
    ...
    {
        $this->pageTitle(trans('plugins/franchise::customer.create'));

        Assets::addScriptsDirectly('vendor/core/plugins/franchise/js/customer.js');

        return FranchiseForm::create()->remove('is_change_password')->renderForm();
    }
    ...
    public function store(CustomerCreateRequest $request)
    {
        ...
        return $this
            ->httpResponse()
            ->setPreviousUrl(route('franchise.customers.index'))
            ->setNextUrl(route('franchise.customers.edit', $customer->getKey()))
            ->withCreatedSuccessMessage();
    }

    public function edit(Franchise $franchise, $id)
    {
        $franchise = Franchise::where('id',$id)->first();
        Assets::addScriptsDirectly('vendor/core/plugins/franchise/js/franchise.js');
        $this->pageTitle(trans('plugins/franchise::franchise.edit', ['name' => $franchise->name]));
        $franchise->password = null;
        return FranchiseForm::createFromModel($franchise)->renderForm();
    }

    public function update(Franchise $customer, Request $request)
    {
        dd($request->all());
