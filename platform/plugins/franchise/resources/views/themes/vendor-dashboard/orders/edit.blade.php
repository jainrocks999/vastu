            ...
            <div class="col-md-8">
                <x-core::card class="mb-3">
                    ...
                    @include('plugins/ecommerce::orders.edit.order-products', [
                        'isInAdmin' => false,
                        'editProductRoute' => 'franchise.vendor.products.edit',
                   ])
                            ...
                            <div class="col-md-6 offset-md-6">
                                ...
                                @if ($order->isInvoiceAvailable())
                                    <div class="text-end my-3">
                                        <x-core::button
                                            tag="a"
                                            :href="route('franchise.vendor.orders.generate-invoice', $order->id)"
                                            target="_blank"
                                            icon="ti ti-download"
                                        >
                                ...
                                @include('plugins/ecommerce::orders.edit.form-edit', ['route' => 'franchise.vendor.orders.edit'])
                            </div>
                        </div>
                    </x-core::card.body>
                    ...
                    <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                        ...
                        @if (!$order->is_confirmed)
                            <x-core::form :url="route('franchise.vendor.orders.confirm')">
                    ...
                    @if(! EcommerceHelper::isDisabledPhysicalProduct())
                        ...
                        @else
                            @include(FranchiseHelper::viewPath('vendor-dashboard.orders.shipment-detail'), [
                                ...
                                <li @class(['step-item', 'user-action' => $history->user_id])>
                                            ...
                                            <x-core::table :striped="false" :hover="false" class="w-100">
                                                <x-core::table.body>
                                                    <x-core::table.body.row>
                                                        ...
                                                        <x-core::table.body.cell>
                                                            <a
                                                                href="{{ route('franchise.vendor.orders.edit', $order->id) }}"
                                                                title="{{ $order->code }}"
                                                            >
                                                                {{ $order->code }}
                                                            </a>
                                    ...
                                    @if ($history->action == 'send_order_confirmation_email')
                                        <x-core::button
                                            ...
                                            :data-action="route('franchise.vendor.orders.send-order-confirmation-email', $history->order_id)"
                                        >
                    ...
                    @if ($order->canBeCanceledByAdmin())
                        <x-core::card.footer>
                            <x-core::button
                                ...
                                :data-target="route('franchise.vendor.orders.cancel', $order->id)"
                            >
...
@pushif ($order->status != Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED, 'footer')
    @include('plugins/ecommerce::orders.edit.modal', [
        'updateShippingAddressRoute' => 'franchise.vendor.orders.update-shipping-address',
    ])
    ...
    @if (! EcommerceHelper::isDisabledPhysicalProduct() && $order->shipment && $order->shipment->id)
        ...
        >
            @include(FranchiseHelper::viewPath('vendor-dashboard.orders.shipping-status-modal'), [
                'shipment' => $order->shipment,
                'url' => route('franchise.vendor.orders.update-shipping-status', $order->shipment->id),
            ])
        </x-core::modal>
    @endif
@endpushif
