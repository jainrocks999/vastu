...
@section('content')
    ...
    <div class="mt-4">
        ...
        <ul class="list-group">
            ...
            @if (FranchiseHelper::getSetting('requires_vendor_documentations_verification', true))
                @if ($store->certificate_file)
                    <li class="list-group-item">
                        <strong>{{ __('Uploaded Certificate') }}: </strong>
                        <a href="{{ route('franchise.vendor.become-vendor.download-certificate') }}" target="_blank" class="text-primary">{{ __('View Certificate') }}</a>
                    </li>
                @endif
                @if ($store->government_id_file)
                    <li class="list-group-item">
                        <strong>{{ __('Uploaded Government ID') }}: </strong>
                        <a href="{{ route('franchise.vendor.become-vendor.download-government-id') }}" target="_blank" class="text-primary">{{ __('View Government ID') }}</a>
                    </li>
                @endif
            @endif
        </ul>
